...
export default function AccountCategory() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await AxiosInstance.get("account-categories/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await AxiosInstance.put(`account-categories/${editingId}/`, formData);
        alert("Updated Successfully!");
      } else {
        await AxiosInstance.post("account-categories/", formData);
        ...
        <table className="w-full border border-collapse text-sm">
          <thead className="bg-blue-950 text-white">
            <tr>
              ...
              <th className="border px-2 py-1">Account Category Name</th>
